...
//   const costPrice = allLeads.reduce((acc, lead) => acc + Number(lead.bouquetCost || 0), 0);

//   // Выручка (сумма salePrice по всем лидам)
//   const orderSum = allLeads.reduce((acc, lead) => acc + Number(lead.salePrice || 0), 0);

//   // Рекламные расходы
//   const expenses = allExpenses.reduce((acc, meta) => acc + meta.adSpend, 0);

//   // Прочие расходы
//   const expensesOther = allExpenses.reduce((acc, meta) => acc + (meta.otherSpend ?? 0), 0);
